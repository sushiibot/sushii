...
describe("GuildConfig", () => {
  ...
  describe("deterministic setter methods", () => {
    ...
    it("should set moderation settings enabled state explicitly", () => {
      ...
      expect(disabledTimeoutCmd.moderationSettings.timeoutCommandDmEnabled).toBe(false);
      
      const disabledTimeoutNative = config.setTimeoutNativeDmEnabled(false);
      expect(disabledTimeoutNative.moderationSettings.timeoutNativeDmEnabled).toBe(false);
    ...
    it("should maintain immutability - original config unchanged", () => {
      ...
      const originalJoinEnabled = originalConfig.messageSettings.joinMessageEnabled;
      
      const modifiedConfig = originalConfig.setJoinMessageEnabled(!originalJoinEnabled);
      
      // Original should be unchanged
      expect(originalConfig.messageSettings.joinMessageEnabled).toBe(originalJoinEnabled);
      // Modified should have the new value
      expect(modifiedConfig.messageSettings.joinMessageEnabled).toBe(!originalJoinEnabled);
    });
    ...
    it("should be deterministic - same input always produces same output", () => {
      ...
      expect(result1.messageSettings.joinMessageEnabled).toBe(result2.messageSettings.joinMessageEnabled);
