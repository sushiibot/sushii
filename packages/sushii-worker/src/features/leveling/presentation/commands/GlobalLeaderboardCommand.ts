...
import type { GetGlobalLeaderboardService, GlobalLeaderboardData } from "../../application/GetGlobalLeaderboardService";
import {
  TimeFrame,
  isValidTimeFrame,
} from "../../domain/value-objects/TimeFrame";
import { buildGlobalLeaderboardContainer } from "../views/GlobalLeaderboardDisplayView";

export default class GlobalLeaderboardCommand extends SlashCommandHandler {
  constructor(
    private readonly getGlobalLeaderboardService: GetGlobalLeaderboardService,
  ) {
    super();
  }
  ...
  async handler(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const paginator = new ComponentsV2Paginator<GlobalLeaderboardData>({
      interaction,
      pageSize: 10,
      callbacks: {
        fetchPage: async (pageIndex, pageSize) => {
          const data =
            await this.getGlobalLeaderboardService.getGlobalLeaderboard(
              interaction.user.id,
              timeframe,
              pageIndex,
              pageSize,
        ...
        renderContainer: (data, state, navButtons) => {
          ...
          return buildGlobalLeaderboardContainer(
            data[0],
            timeframe,
            interaction.user.id,
            navButtons,
