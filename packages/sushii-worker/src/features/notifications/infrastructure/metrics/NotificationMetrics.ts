...
import type { Gauge, Counter } from "@opentelemetry/api";
...
export class NotificationMetrics {
  ...
  constructor() {
    try {
      ...
      this.activeNotificationsGauge = meter.createGauge("notifications_pending", {
        description: "Active keyword notifications",
        valueType: ValueType.INT,
      });

      this.sentNotificationsCounter = meter.createCounter("notifications_sent_count", {
      ...
      logger.error({ err: error }, "Failed to initialize NotificationMetrics - OTEL SDK may not be initialized yet");
      throw error;
    }
  }
}