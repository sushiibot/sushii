 ...
 * page, oldest-at-top within a page — rendering with the tab's
 ...
 * order: pages run newest-first while rows within a page run oldest-at-top,
 * so any ordering sentence here would contradict what the eye sees.
 * Per-entry relative timestamps already make ordering self-evident.
 */
function buildScopeText(
  ...
  // One merged line, not three separate whispers.
 ...
 * History is paginated (`chunkItems`) and drops nothing; the pagination
 * wiring owns page turns and must rebuild the full frame via
...
export const addHistoryTabContent: ModViewTabContentBuilder = (
  ...
  if (hasLinkedIdentity) {
    addScopeBlockWithAction(container, scopeText, {
      customId: MODVIEW_CUSTOM_IDS.historyAlts,
      label: includeAlts ? "Hide Alts" : "Show Alts",
      disabled,
    });
  } else {
    addSubtextBlock(container, scopeText);
  }
