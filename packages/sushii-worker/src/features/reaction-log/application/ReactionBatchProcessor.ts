...
export class ReactionBatchProcessor {
  ...
  async queueReactionEvent(event: ReactionEvent): Promise<void> {
    ...
    try {
      ...
      if (event.type === 'add') {
        ...
        this.logger.trace(
          { messageId: event.messageId, emoji: event.emoji, userId: event.userId, isInitial: event.isInitial },
      ...
      this.logger.error(
        { err, event },
        "Failed to queue reaction event"
      );
    }
  }
  ...
  private async processBatch(messageKey: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        { err, batch },
        "Failed to process reaction batch"
      );
    } finally {
