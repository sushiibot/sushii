...
import type { ReactionBatch, ReactionEvent } from "../../domain/types/ReactionEvent";

export function createReactionLogMessage(batch: ReactionBatch): { embeds: EmbedBuilder[] } {
...
function formatRemoves(groups: EmojiGroup[]): string {
  ...
  for (const group of groups) {
    ...
    if (starterRemovals.length > 0) {
      const starterUsers = starterRemovals
        .map(action => {
  ...
  for (const [_, userActions] of userEmojiActions) {
    if (userActions.length >= 3) {
      ...
      for (let i = 0; i < userActions.length - 2; i++) {
        const timeDiff = userActions[i + 2].timestamp.getTime() - userActions[i].timestamp.getTime();
