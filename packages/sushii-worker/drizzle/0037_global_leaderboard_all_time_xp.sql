...
CREATE MATERIALIZED VIEW "app_public"."global_user_level_rankings_all_time" AS (SELECT user_id, SUM(msg_all_time) AS all_time_xp, DENSE_RANK() OVER (ORDER BY SUM(msg_all_time) DESC, user_id DESC) AS rank FROM app_public.user_levels GROUP BY user_id);
